...
The goal of this quick start guide is to index all transfers and approval events for Wrapped Monad (`0x760AfE86e5de5fa0Ee542fc7B7B713e1c5425701`) on Monad Testnet.

<!-- @include: ../snippets/evm-quickstart-reference.md -->

We use Ethereum packages, runtimes, and handlers (e.g. `@subql/node-ethereum`, `ethereum/Runtime`, and `ethereum/*Hander`) for Monad. Since Monad is an EVM-compatible layer-1, we can use the core Ethereum framework to index it.
:::

<!-- @include: ../snippets/evm-manifest-intro.md#level2 -->

As we are indexing all transfers and approvals from the Wrapped Monad contract on Monad's Testnet, the first step is to import the contract abi definition which can be obtained from from any standard [ERC-20 contract](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/). Copy the entire contract ABI and save it as a file called `erc20.abi.json` in the `/abis` directory.
  ...
  dataSources: [
    {
      ...
      options: {
        abi: "erc20",
        // This is the contract address for Wrapped Monad
      ...
      mapping: {
        ...
        handlers: [
          {
            kind: EthereumHandlerKind.Call, // We use ethereum handlers since Monad Testnet is EVM-compatible
...
The above code indicates that you will be running a `handleTransaction` mapping function whenever there is a `approve` method being called on any transaction from the [Wrapped Monad contract](https://testnet.monadexplorer.com/token/0x760AfE86e5de5fa0Ee542fc7B7B713e1c5425701).

The code also indicates that you will be running a `handleLog` mapping function whenever there is a `Transfer` event being emitted from the [Wrapped Monad contract](https://testnet.monadexplorer.com/token/0x760AfE86e5de5fa0Ee542fc7B7B713e1c5425701).
...
::: tip Note
The final code of this project can be found [here](https://github.com/subquery/ethereum-subql-starter/tree/main/Monad/monad-testnet-starter).
:::
