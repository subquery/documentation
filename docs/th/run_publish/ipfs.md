...
На данный момент, чтобы просмотреть опубликованный проект, используйте инструмент `REST` api, например [Postman](https://web.postman.co/), и используйте метод `POST` со следующим примером URL: `https://subquery.network/ipfs/api/v0/cat?arg=<YOUR_PROJECT_CID>`
