...
These RPC methods are an extension to the existing [Ethereum JSON-RPC](https://ethereum.org/en/developers/docs/apis/json-rpc). They can be enabled in SubQuery Data Nodes by following [these instructions.](./introduction#Running).
...
Returns information about the capabilities of filtering blocks
...
The `supportedResponses` determines the behaviour of field selectors. If `complete` is supported then it is possible to get all fields on transactions and logs. If `basic` is the only option then only header information is selectable.
...
1. OBJECT - Filter options
  * fromBlock: QUANTITY - Hex encoded block number
  * toBlock: QUANTITY - Hex encoded block number
  * limit: QUANTITY - Hex encoded number
  * filter: BLOCK_FILTER - This determines the blocks returned as well as the content within the blocks. TODO define filter matching behaviour including null, *, case sensitivity etc
    ```ts
    ...
    ```
  * fieldSelector: FIELD_SELECTOR - (Optional) - Specifies the fields returned in the block response. If this is undefined then just the header will be returned.
    ```ts
...
true or {} will return minimal fields for an object. e.g logs: true would return logIndex, transactionIndex so that it could be associated to the relevant transaction
...
1. BLOCK_RESULT - Contains information about the blocks that match the filter as well as the block range searched.
