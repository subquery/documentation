...
The SubQuery store is an injected class that allows users to interact with record in the database from within mapping functions. This will come handy when user demand use multiple entity records as the parameters in the mapping function, or create/update multiple records in a single place.
