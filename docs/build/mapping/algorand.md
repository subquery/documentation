...
There are different classes of mappings functions for Algorand: [Block handlers](mapping.md#block-handler), and [Transaction Handlers](mapping.md#transaction-handler).
...
export async function handleTransaction(
  ...
  const record = new TransactionEntity(tx.id);
