...
There are different classes of mappings functions for Polkadot/Substrate; [Block handlers](mapping.md#block-handler), [Event Handlers](mapping.md#event-handler), and [Call Handlers](mapping.md#call-handler).
...
Call handlers are used when you want to capture information on certain substrate extrinsics. You should use [Mapping Filters](../build/manifest/polkadot.md#mapping-handlers-and-filters) in your manifest to filter calls to reduce the time it takes to index data and improve mapping performance.
...
We also injected a `Store` object which allow users to interact with records in the database. 
This will come handy when user demand use multiple entity records as the parameters in the mapping function, or create/update multiple records in a single place.

Following is an example of `Store` interface
...
This allows get list of entities with its `id`.

```typescript
await store.getByField(`StarterEntity`,'field1',50)
```

Please note, the third parameter also accept array, you can consider this similar like `bulkGet`.
...
This allows get a single record with matching field and value.
```typescript
const field1Value = 50;
await store.get(`StarterEntity`,`field1`,field1Value)
```
...
#### store.bulkCreate
This allows to create multiple records for specified entity, but it **won't*** overwrite existing records.
...
#### store.bulkUpdate
This allows to update multiple records for specified entity, if create the records if they are not exist. 
...
```
Please note, this fields feature currently not working with historical project, it will overwrite all attributes.
