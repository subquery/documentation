# BNB Smart Chain (BSC) Mapping

::: warning Important
We use Ethereum types (e.g. `EthereumBlock`, `EthereumTransaction`, and `EthereumLog`) for BNB Smart Chain (BSC). Since BSC is a layer-2 scaling solution, we can use the core Ethereum framework to index it.
:::
...
```ts
import { EthereumBlock } from "@subql/types-ethereum"; // We use ethereum types since BSC is a layer-2 that is compatible
...
You can use transaction handlers to capture information about each of the transactions in a block. To achieve this, a defined TransactionHandler will be called once for every transaction. You should use [Mapping Filters](../manifest/bsc.md#mapping-handlers-and-filters) in your manifest to filter transactions to reduce the time it takes to index data and improve mapping performance.
